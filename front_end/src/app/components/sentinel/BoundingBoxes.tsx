...
export default function BoundingBoxes({ detections, bodies = [], videoWidth, videoHeight, onSelectDetection }: Props) {
  ...
  return (
    <svg
      viewBox={`0 0 ${videoWidth || 640} ${videoHeight || 480}`}
    ...
    >
      ...
      {detections.map((det, i) => {
        ...
        const isConfirmed = det.matched && (det.confirmed ?? true);
        ...
        return (
          <g key={faceKey} onClick={() => onSelectDetection?.(det)} style={{ cursor: onSelectDetection ? "pointer" : "default", pointerEvents: "auto" }}>
