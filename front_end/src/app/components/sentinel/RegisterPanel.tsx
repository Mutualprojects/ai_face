...
export default function RegisterPanel({ onSuccess, canCapture, captureFrame }: Props) {
  ...
  const s: Record<string, React.CSSProperties> = {
    ...
    label:   { fontSize:10, fontWeight:700, letterSpacing:"0.1em", color:"#6b7280", textTransform:"uppercase", marginBottom:4 },
    input:   { width:"100%", background:"#f9fafb", border:"1px solid #d1d5db",
                borderRadius:10, padding:"10px 14px", color:"#111827", fontSize:14, outline:"none" },
    btnPri:  { background:"linear-gradient(135deg,#6366f1,#4f46e5)", color:"#ffffff", border:"none",
    ...
    btnSec:  { background:"#f3f4f6", color:"#374151", border:"1px solid #d1d5db",
  ...
  return (
    <div style={s.panel}>
      <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:4 }}>
        <div style={{ width:28, height:28, borderRadius:8, background:"#6366f115",
        ...
        <span style={{ fontWeight:700, fontSize:13, color:"#111827" }}>Register New Face</span>
      </div>

      {/* Image preview / capture area */}
      <div style={{ aspectRatio:"4/3", borderRadius:14, overflow:"hidden", background:"#f9fafb",
        border:"1px dashed #d1d5db", position:"relative", display:"flex",
        ...
        {image ? (
          <>
            ...
            <button onClick={reset} style={{ position:"absolute", top:8, right:8,
              ...
              color:"#111827", borderRadius:8, padding:"4px 10px", fontSize:11, cursor:"pointer", fontWeight:700 }}>
              ✕ Clear
            </button>
          </>
        ) : (
          <div style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:12, padding:20, textAlign:"center" }}>
            <div style={{ width:48, height:48, borderRadius:"50%", border:"2px dashed #9ca3af",
              display:"flex", alignItems:"center", justifyContent:"center" }}>
            ...
            <div style={{ display:"flex", gap:8 }}>
              <button onClick={capture} disabled={!canCapture} suppressHydrationWarning style={{...s.btnSec,
                color: canCapture ? "#059669" : "#9ca3af",
                borderColor: canCapture ? "#10b981" : "#e5e7eb" }}>
                📷 Capture
              </button>
      ...
      <form onSubmit={submit} style={{ display:"flex", flexDirection:"column", gap:12 }}>
        ...
        {error && <p style={{ fontSize:12, color:"#ef4444", margin:0, fontWeight:600 }}>⚠ {error}</p>}
        {success && <p style={{ fontSize:12, color:"#10b981", margin:0, fontWeight:600 }}>{success}</p>}
