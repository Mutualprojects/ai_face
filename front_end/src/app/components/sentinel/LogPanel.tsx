...
export default function LogPanel({ logs, faces, onRefresh }: Props) {
  const s: Record<string, React.CSSProperties> = {
    ...
    icon: {
      width: 28, height: 28, borderRadius: 8, background: "rgba(255,179,0,0.12)",
    ...
    avatarPH: {
      width: 44, height: 44, borderRadius: 8, flexShrink: 0, background: "rgba(255,255,255,0.05)",
      display: "flex", alignItems: "center", justifyContent: "center"
    },
    comparisonContainer: {
    ...
    <div>
      ...
      {logs.length === 0 ? (
        ...
        <div style={{ maxHeight: 340, overflowY: "auto" }}>
          {logs.map(log => {
            const isKnown = log.person_name !== "Unknown" && !log.person_name.startsWith("Unknown (Closest:");
            const ts = log.created_at || log.timestamp;
            
            let lookupName = log.person_name;
            let displayName = log.person_name;
            if (log.person_name.startsWith("Unknown (Closest: ")) {
              lookupName = log.person_name.substring("Unknown (Closest: ".length, log.person_name.length - 1);
            }
            const matchedFace = faces.find(f => f.name.toLowerCase() === lookupName.toLowerCase());
              ...
              <div key={log.id} style={s.row}>
                ...
                <div style={s.comparisonContainer}>
                  ...
                  <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 2 }}>
                    ...
                    <span style={{
                      fontSize: 8, fontWeight: 700, color: isKnown ? "var(--green)" : "var(--red)",
