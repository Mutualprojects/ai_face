...
type ViewMode = "cards" | "list" | "table" | "kanban" | "calendar";
...
export default function LogPanel({ logs, faces, onRefresh, autoRefresh = false }: Props) {
  ...
  const now = new Date();
  const [calMonth, setCalMonth] = useState<{ y: number; m: number }>({ y: now.getFullYear(), m: now.getMonth() });
  ...
  const calLogsByDay = useMemo(() => {
    const map: Record<string, EnrichedFaceLog[]> = {};
    for (const l of filteredAndSortedLogs) {
  ...
  const calMonthStats = useMemo(() => {
    ...
    for (const l of filteredAndSortedLogs) {
      ...
      if (!ts) continue;
      const d = new Date(ts);
      if (d.getFullYear() !== calMonth.y || d.getMonth() !== calMonth.m) continue;
      if (l.person_name !== "Unknown") matched++;
      else unknown++;
    }
    return { matched, unknown, total: matched + unknown };
  }, [filteredAndSortedLogs, calMonth]);
      ...
      {filteredAndSortedLogs.length === 0 ? (
        ...
        <div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", gap: 6 }}>
            {calCells.map((date) => {
              const inMonth = date.getMonth() === calMonth.m;
              const k = dayKey(date);
              const dayLogs = calLogsByDay[k] || [];
              const matched = dayLogs.filter(l => l.person_name !== "Unknown").length;
              const unknown = dayLogs.length - matched;
              const isSelected = calSelectedDay !== null && dayKey(calSelectedDay) === k;
              const isToday = dayKey(new Date()) === k;

              return (
                <button
                  key={k}
                  onClick={() => setCalSelectedDay(isSelected ? null : date)}
                ...
                >
                  <span style={{ fontSize: 11, fontWeight: 800, color: isSelected ? "#4338ca" : isToday ? "#4f46e5" : "#334155" }}>
                    {date.getDate()}
                  </span>
                  {dayLogs.length > 0 && (
                    <>
                      {matched > 0 && (
                        <span style={{ fontSize: 9, fontWeight: 700, color: "#059669", background: "rgba(16,185,129,0.12)", borderRadius: 5, padding: "1px 5px", width: "fit-content", display: "flex", alignItems: "center", gap: 3 }}>
                          <span style={{ width: 5, height: 5, borderRadius: "50%", background: "#10b981" }} />
                          {matched} matched
                        </span>
                      )}
                      {unknown > 0 && (
                        <span style={{ fontSize: 9, fontWeight: 700, color: "#dc2626", background: "rgba(239,68,68,0.12)", borderRadius: 5, padding: "1px 5px", width: "fit-content", display: "flex", alignItems: "center", gap: 3 }}>
                      ...
                      return (
                        <div key={log.id} style={{
                          ...
                          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                            ...
                            <span style={{ fontSize: 9, fontWeight: 700, padding: "2px 8px", borderRadius: 10, color: isKnown ? "#059669" : "#dc2626", background: isKnown ? "rgba(16,185,129,0.1)" : "rgba(239,68,68,0.1)" }}>
