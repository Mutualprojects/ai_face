...
export default function GalleryPanel({ faces, onDelete }: Props) {
  const s: Record<string, React.CSSProperties> = {
    ...
    icon:   { width:28, height:28, borderRadius:8, background:"rgba(6,182,212,0.15)",
    ...
    name:   { fontSize:11, fontWeight:700, color:"var(--text-primary)", textAlign:"center" as const,
               overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" as const, width:"100%" },
    del:    { position:"absolute" as const, top:6, right:6, background:"rgba(255,59,92,0.15)",
               border:"1px solid rgba(255,59,92,0.3)", borderRadius:6, color:"#ff3b5c",
    ...
    empty:  { display:"flex", flexDirection:"column" as const, alignItems:"center", gap:8,
               padding:"32px 16px", color:"var(--text-muted)", fontSize:12, textAlign:"center" as const },
  };
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:0 }}>
      <div style={s.header}>
        ...
        <span style={{ marginLeft:"auto", fontSize:11, background:"rgba(124,58,237,0.2)",
          color:"#a78bfa", borderRadius:20, padding:"2px 8px", fontWeight:700 }}>
          {faces.length}
        </span>
      </div>
          ...
          {faces.map(f => (
            <div key={f.id} style={s.card}>
              {f.photo_url
                ...
                : <div style={{ ...s.avatar, background:"rgba(124,58,237,0.2)",
                    display:"flex", alignItems:"center", justifyContent:"center" }}>
